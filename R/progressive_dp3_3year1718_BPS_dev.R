#~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Shiny app to investigate the impact of direct payment withdrawl on farming incomes 
# reads in data processed in progressive_dp3_data_processing_1718.R 
...
#This calculates the loss for each business using RPA data based on the Total_Payment_pounds
  #observeEvent(input$goButton,{
    ...
    rpaloss$pay_band_money_lost <- cut(rpaloss$loss,
  ...
  Loss_fbs <- reactive ({
    ...
    fbsloss$loss <- dplyr::case_when(
      ...
      # if payment is between level 5 and level 6
      dplyr::between(fbsloss$dp_3yr, input$Level_5+1, input$Level_6) ~ 
        band_6_reduction(fbsloss$dp_3yr),
      # if payment is between level 6 and level 7
      dplyr::between(fbsloss$dp_3yr, input$Level_6+1, input$Level_7) ~ 
        band_7_reduction(fbsloss$dp_3yr),
      # if payment is between level 7 and level 8
      dplyr::between(fbsloss$dp_3yr, input$Level_7+1, input$Level_8) ~ 
        band_8_reduction(fbsloss$dp_3yr),
      # if payment is between level 8 and level 9
      dplyr::between(fbsloss$dp_3yr, input$Level_8+1, input$Level_9) ~ 
        band_9_reduction(fbsloss$dp_3yr),
      # if payment is between level 9 and level 10
      dplyr::between(fbsloss$dp_3yr, input$Level_9+1, input$Level_10) ~ 
        band_10_reduction(fbsloss$dp_3yr),
      # if payment is between level 10 and level 11
      dplyr::between(fbsloss$dp_3yr, input$Level_10+1, input$Level_11) ~ 
        band_11_reduction(fbsloss$dp_3yr),
      # if payment is between level 11 and level 12
      dplyr::between(fbsloss$dp_3yr, input$Level_11+1, input$Level_12) ~ 
  ...
  #~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  # Total payment loss for all farms

  output$table_1 <- renderDataTable({
    FBSCore::total_byfac("loss",
                         input$fbsfac,
                         design = fbsdesign)[, c(1, 2, 5)] %>%
      bind_cols(data.frame(
        mosaic::sum(num_affected ~ type, data = fbsloss)
      ) %>%
        rbind(sum(fbsloss$num_affected)),
      FBSCore::total_byfac("dp_3yr",
                           input$fbsfac,
                           design = fbsdesign)[, c(2, 5)]) %>%
      mutate('proportional loss of DP' = scales::percent((loss / dp_3yr), accuracy = 1)) %>%
      dplyr::rename('Total payment loss (£)' = loss,
                    '±95% CI payment loss' = Ci.loss,
                    'num in sample with payment loss' = mosaic..sum.num_affected...type..data...fbsloss.,
                    'Total original payment (£)' = dp_3yr,
                    '±95% CI original payment' = Ci.dp_3yr)
  })  
