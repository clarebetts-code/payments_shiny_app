...
#This calculates the loss for each business using RPA data based on the TOTAL PAYMENT
  #observeEvent(input$goButton,{
    Loss_rpa <- reactive ({
      #####
      rpaloss <- subset(RPApay_interim,RPApay_interim$Total_Payment_EUR>0)
      rpaloss$loss <- case_when(
        # if payment is less than first level, reduce by prop_1
        rpaloss$Total_Payment_EUR < input$Level_1 ~ 
          band_1_reduction(rpaloss$Total_Payment_EUR),
        # if payment is between level 1 and level 2
        between(rpaloss$Total_Payment_EUR, input$Level_1, input$Level_2) ~ 
          band_2_reduction(rpaloss$Total_Payment_EUR),
        # if payment is between level 2 and level 3
        between(rpaloss$Total_Payment_EUR, input$Level_2, input$Level_3) ~ 
          band_3_reduction(rpaloss$Total_Payment_EUR),
        # if payment is between level 3 and level 4
        between(rpaloss$Total_Payment_EUR, input$Level_3, input$Level_4) ~ 
          band_4_reduction(rpaloss$Total_Payment_EUR),
        # if payment is between level 4 and level 5
        between(rpaloss$Total_Payment_EUR, input$Level_4, input$Level_5) ~ 
          band_5_reduction(rpaloss$Total_Payment_EUR),
        # if payment is between level 5 and level 6
        between(rpaloss$Total_Payment_EUR, input$Level_5, input$Level_6) ~ 
          band_6_reduction(rpaloss$Total_Payment_EUR),
        # if payment is between level 6 and level 7
        between(rpaloss$Total_Payment_EUR, input$Level_6, input$Level_7) ~ 
          band_7_reduction(rpaloss$Total_Payment_EUR),
        # if payment is between level 7 and level 8
        between(rpaloss$Total_Payment_EUR, input$Level_7, input$Level_8) ~ 
          band_8_reduction(rpaloss$Total_Payment_EUR),
        # if payment is between level 8 and level 9
        between(rpaloss$Total_Payment_EUR, input$Level_8, input$Level_9) ~ 
          band_9_reduction(rpaloss$Total_Payment_EUR),
        # if payment is between level 9 and level 10
        between(rpaloss$Total_Payment_EUR, input$Level_9, input$Level_10) ~ 
          band_10_reduction(rpaloss$Total_Payment_EUR),
        # if payment is between level 10 and level 11
        between(rpaloss$Total_Payment_EUR, input$Level_10, input$Level_11) ~ 
          band_11_reduction(rpaloss$Total_Payment_EUR),
        # if payment is between level 11 and level 12
        between(rpaloss$Total_Payment_EUR, input$Level_11, input$Level_12) ~ 
          band_12_reduction(rpaloss$Total_Payment_EUR)
        )
    
    #Set the bands for analysis
    rpaloss$pay_band <- as.factor(cut(rpaloss$Total_Payment_EUR, 
  ...
  Loss_fbs <- reactive ({
    #####
    fbsloss <- fbs_3yr
    fbsloss$loss <- case_when(
      # if payment is less than first level, reduce by prop_1
      fbsloss$dp_3yr < input$Level_1 ~ 
        band_1_reduction(fbsloss$dp_3yr),
      # if payment is between level 1 and level 2
      between(fbsloss$dp_3yr, input$Level_1, input$Level_2) ~ 
        band_2_reduction(fbsloss$dp_3yr),
      # if payment is between level 2 and level 3
      between(fbsloss$dp_3yr, input$Level_2, input$Level_3) ~ 
        band_3_reduction(fbsloss$dp_3yr),
      # if payment is between level 3 and level 4
      between(fbsloss$dp_3yr, input$Level_3, input$Level_4) ~ 
        band_4_reduction(fbsloss$dp_3yr),
      # if payment is between level 4 and level 5
      between(fbsloss$dp_3yr, input$Level_4, input$Level_5) ~ 
        band_5_reduction(fbsloss$dp_3yr),
      # if payment is between level 5 and level 6
      between(fbsloss$dp_3yr, input$Level_5, input$Level_6) ~ 
        band_6_reduction(fbsloss$dp_3yr),
      # if payment is between level 6 and level 7
      between(fbsloss$dp_3yr, input$Level_6, input$Level_7) ~ 
        band_7_reduction(fbsloss$dp_3yr),
      # if payment is between level 7 and level 8
      between(fbsloss$dp_3yr, input$Level_7, input$Level_8) ~ 
        band_8_reduction(fbsloss$dp_3yr),
      # if payment is between level 8 and level 9
      between(fbsloss$dp_3yr, input$Level_8, input$Level_9) ~ 
        band_9_reduction(fbsloss$dp_3yr),
      # if payment is between level 9 and level 10
      between(fbsloss$dp_3yr, input$Level_9, input$Level_10) ~ 
        band_10_reduction(fbsloss$dp_3yr),
      # if payment is between level 10 and level 11
      between(fbsloss$dp_3yr, input$Level_10, input$Level_11) ~ 
        band_11_reduction(fbsloss$dp_3yr),
      # if payment is between level 11 and level 12
      between(fbsloss$dp_3yr, input$Level_11, input$Level_12) ~ 
  ...
  data_var2 <- reactive({
    ...
    setnames(stat_byfac,"loss","Total payment loss (£m)")
    setnames(stat_byfac,"tots_var2()[, 2]","Total Orig. Dir. Payment (£m)")
  ...
  for_dp_chart <- reactive({
    chart <- data_var2()
    chart$lossdist <- chart$`Total payment loss (£m)`/sum(chart$`Total payment loss (£m)`) * 100
    chart$dpdist <- chart$`Total Orig. Dir. Payment (£m)`/sum(chart$`Total Orig. Dir. Payment (£m)`) * 100
  ...
  output$caption1 <- renderText({
    paste("RPA Saved: £",format(round(sum(Loss_rpa()$loss),0),big.mark=","),
          "(",
          format(round(100*sum(Loss_rpa()$loss/sum(Loss_rpa()$Total_Payment_EUR),0))),
          "%) from ",
  ...
  output$caption1bps <- renderText({
    paste("RPA Saved: £",format(round(sum(Loss_rpa_bps()$loss),0),big.mark=","),
          "(",
          format(round(100*sum(Loss_rpa_bps()$loss/sum(Loss_rpa_bps()$Total_Payment_EUR),0))),
          "%) from ",
  ...
  output$caption1a <- renderText({
    paste("Total paid in 2016 = £",
          format(round(sum(Loss_rpa()$Total_Payment_EUR),0),big.mark=","),
          "to ",
...
#  })
  ...
  output$plot3 <- renderPlot({
    ggplot(aggdata(), aes(x = pay_band, y=loss_mill))+
      ...
      theme_bw()+
      xlab("Total Direct Payment band")+ylab("Loss (£ million)")+
      geom_text(aes(label=paste0("£",round(loss_mill,1),"m")), vjust=-0.5,color = "black", size=4.5)+
